    ...
    constructor(data: APIAvatarInfo) {
        ...
        this.constellations =
            characterInfo.constellationIds.map((id) => {
                return new CharacterConstellation(
                    id,
                    !data.talentIdList?.includes(id),
                )
            }) || []
        ...
        const weaponData = data.equipList.find(
            (equip): equip is APIWeaponEquip =>
                equip.flat.itemType === 'ITEM_WEAPON',
        )
        ...
        this.weapon = new Weapon(
            ...
            [0, 20, 40, 50, 60, 70, 80, 90][
                weaponData.weapon.promoteLevel ?? 0
            ] >= weaponData.weapon.level,
