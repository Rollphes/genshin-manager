...
> {
   ...
   * @returns A new QueryLocation instance
   */
  public static create<T extends string, N extends string>(
    type: T,
    name: N,
  ): QueryLocation<T, N> {
    return new QueryLocation<T, N>(type, name, [
   ...
   * @returns A new QueryLocation instance with the property segment appended
   */
  public prop(name: string): QueryLocation<TType, TName> {
    return new QueryLocation<TType, TName>(this.sourceType, this.sourceName, [
      ...this.segments,
   ...
   * @returns A new QueryLocation instance with the index segment appended
   */
  public index(idx: number): QueryLocation<TType, TName> {
    return new QueryLocation<TType, TName>(this.sourceType, this.sourceName, [
      ...this.segments,
   ...
   * @returns A new QueryLocation instance with the filter segment appended
   */
  public filter(
    key: string,
    value: string | number,
  ): QueryLocation<TType, TName> {
    return new QueryLocation<TType, TName>(this.sourceType, this.sourceName, [
      ...this.segments,
