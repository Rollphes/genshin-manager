...
/**
 * UID validation schema (string, 9 digits)
 */
...
/**
 * Enka UID validation schema (number, 9-10 digits)
 */
export const enkaUidSchema = z
  .number()
  .refine((uid) => /^1?\d{9}$/.test(String(uid)), {
    message: 'UID must be a 9-10 digit number',
  })
